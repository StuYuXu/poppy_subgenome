...
##both_conservation
both_retain <- read.csv("/COR/all.csv",stringsAsFactors = F)
...
write.table(gene_distance_1kb_dup_con,"/conv/gene_upstream_1kb_all.bed",
            sep = "\t",col.names = F,row.names = F,quote = F)

write.table(gene_distance_1kb_dup_spe,"/spe/gene_upstream_1kb_all.bed",
            sep = "\t",col.names = F,row.names = F,quote = F)
...
cns_overlap <- read.csv("/conv/cns_overlap_1kb.bed",stringsAsFactors = F,sep="\t",header = F)
cns_overlap <- cns_overlap[order(cns_overlap$V1,cns_overlap$V2,cns_overlap$V3),]
cns_overlap[,5] <- 1:15715
cns_overlap[,4] <- paste0(cns_overlap[,4],"_",cns_overlap[,5])
cns_overlap <- cns_overlap[,-5]
write.table(cns_overlap,"/conv/cns_overlap_1kb_modify.bed",
            sep = "\t",col.names = F,row.names = F,quote = F)

cns_overlap <- read.csv("/spe/cns_overlap_1kb.bed",stringsAsFactors = F,sep="\t",header = F)
cns_overlap <- cns_overlap[order(cns_overlap$V1,cns_overlap$V2,cns_overlap$V3),]
cns_overlap[,5] <- 1:7225
cns_overlap[,4] <- paste0(cns_overlap[,4],"_",cns_overlap[,5])
cns_overlap <- cns_overlap[,-5]
write.table(cns_overlap,"/spe/cns_overlap_1kb_modify.bed",
            sep = "\t",col.names = F,row.names = F,quote = F)
...
ls | awk '{print("annotatePeaks.pl ./shuffle/"$1" Papaver_somniferum.ref.fa -m ../arab.dap.motifs > ./shuffle_motif/"$1".txt")}' > ../command_all
